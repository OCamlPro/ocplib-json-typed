...
let schema encoding =
  ...
  and schema
    ...
    = function
      ...
      | Constant str -> let len = String.length str in
        { title = None ;
          description = None ;
          default = None ;
          enum = Some [ Json_repr.(repr_to_any (module Ezjsonm)) (`String str) ] ;
          kind = String { pattern = Some ("^" ^ str ^ "$") ; min_length = len ; max_length = Some len } ;
          format = None ;
          id = None }
